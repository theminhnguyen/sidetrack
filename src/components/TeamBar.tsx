...
function UserEditPanel({ userId, close }: { userId: string; close: () => void }) {
  ...
  return (
    <div className="flex flex-col gap-3 text-sm">
      <div>
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="shrink-0 rounded-md border border-white/15 px-2 text-xs hover:bg-white/10"
          >
            Save
          </button>
        </div>
      ...
      <div>
        <label className="mb-1 block text-xs text-white/50">Day-job capacity</label>
      ...
      <div>
        <label className="mb-1 block text-xs text-white/50">Note (optional)</label>
        <input
          value={note}
      ...
      <button
        ...
        className="self-start text-xs text-rose-400/80 hover:text-rose-300"
      >
        Mark as inactive
      </button>
    </div>
...
function AddTeammatePanel({ close }: { close: () => void }) {
  ...
  return (
    ...
    >
      <label className="text-xs text-white/50">Teammate name</label>
      <input
        autoFocus
        value={name}
        onChange={(e) => setName(e.target.value)}
      ...
      <button
        type="submit"
        className="self-start rounded-md border border-white/15 px-3 py-1 text-xs hover:bg-white/10"
      >
        Add
      </button>
    </form>
    ...
    <div className="flex flex-wrap items-center gap-3">
      {users.map((user) => (
        <Popover
          ...
          trigger={(open) => (
            <button
              onClick={open}
              className="flex items-center gap-2 rounded-full border border-white/15 py-1 pl-1 pr-3 hover:bg-white/5"
            >
              <Avatar user={user} />
      ...
      <Popover
        trigger={(open) => (
          <button
            onClick={open}
            className="flex items-center gap-1 rounded-full border border-dashed border-white/20 px-3 py-1.5 text-sm text-white/60 hover:bg-white/5"
          >
            + Add teammate
          </button>
        )}
