...
export function DependencyPicker({
  ...
  return (
    <div>
      {selected.length > 0 && (
        <div className="mb-2 flex flex-wrap gap-1.5">
          {selected.map((t) => (
            <span
              key={t.id}
              className="flex items-center gap-1.5 rounded-full border border-white/15 bg-white/5 px-2 py-0.5 text-xs"
            >
              {t.title}
              <button
      ...
      />
      {query.trim() && (
        <div className="mt-1 max-h-40 overflow-y-auto rounded-md border border-white/10">
          {candidates.length === 0 && (
            <p className="px-2 py-1.5 text-xs text-white/40">No matching tasks</p>
          )}
          {candidates.map((t) => (
            <button
              key={t.id}
